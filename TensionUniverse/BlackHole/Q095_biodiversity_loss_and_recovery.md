...
The TU goal is not to replace ecological science, but to give an effective layer encoding where these questions map to:
...
3. Thresholds and hysteresis
   Many ecosystems show regime shifts, extinction debts, and recovery debts. This means that:
...
These difficulties make it impossible, at present, to give a closed form solution or a single agreed predictive model. Instead, Q095 is treated as an S rank problem where:
...
3. It acts as a bridge between:
   ...
   * climate centric problems (for example Q091 climate sensitivity, Q092 tipping points, Q093 carbon cycle),
   * socio technical problems (for example Q098 Anthropocene system dynamics),
   * global risk problems (for example Q080 biosphere limits, Q100 pandemic risk, Q105 systemic crashes).
...
* Q092
  Reason: both Q092 and Q095 describe systems that can cross thresholds into degraded regimes with long, path dependent recovery.
...
We introduce the following observables, each treated as a well defined map on `M_reg` as defined later.
...
We do not specify how these scalars are computed internally. We only require that they are well defined, nonnegative, and depend only on the information already encoded in `m`.

### 3.3 Risk tail biodiversity tension functional

We define a risk tail oriented biodiversity tension functional:

```txt
Tension_Bio(m) =
    w_loss * DeltaS_loss(m) +
    w_recovery * DeltaS_recovery(m) +
    w_driver * G_driver(D_driver(m))
```

where:

* `w_loss`, `w_recovery`, and `w_driver` are fixed nonnegative weights with:

  ```txt
  w_loss + w_recovery + w_driver = 1
  ```

  chosen once for this encoding and not adjusted per dataset or per scenario.

* `G_driver(D_driver(m))` is a nonnegative scalar summarising overall driver pressure from the driver vector. For example `G_driver` can be a weighted sum:

  ```txt
  G_driver(D_driver(m)) =
      a_land * D_land(m) +
      a_climate * D_climate(m) +
      a_exploit * D_exploit(m) +
      a_pollution * D_pollution(m) +
      a_invasive * D_invasive(m)
  ```

  with fixed nonnegative coefficients `a_land`, `a_climate`, `a_exploit`, `a_pollution`, `a_invasive` chosen in advance.

Properties required at the effective layer:

* `Tension_Bio(m) >= 0` for all `m` in the regular domain.
* If `DeltaS_loss(m)` and `DeltaS_recovery(m)` are small and driver pressure is low, then `Tension_Bio(m)` is small.
* If `DeltaS_loss(m)` and `DeltaS_recovery(m)` are large or driver pressure is high, then `Tension_Bio(m)` is large.

The choice of weights and the form of `G_driver` is part of the encoding. Once chosen for a given analysis, these parameters are held fixed during experiments and not tuned after seeing the outcomes.

### 3.4 Invariants and effective constraints

We define two simple invariants at the effective layer.
...
```txt
I_intact(m_intact) is close to 0
```

within a tolerance band that depends on measurement noise and natural variability.
...
* `I_recovery(m)` near 1 indicates high recovery feasibility,
* `I_recovery(m)` near 0 indicates low or zero recovery feasibility.

These invariants do not encode any deep dynamics. They simply convert the mismatch scalars into interpretable effective layer quantities.
...
All tension analysis for Q095 is restricted to `M_reg`. When an experiment encounters a state in `S_sing`, the outcome is treated as:

* out of domain for `Tension_Bio`,
...
This block describes how Q095 is characterized as a tension problem within TU at the effective layer.
...
* a risk tail oriented tension functional `Tension_Bio(m)` that highlights states where:
...
> the problem of defining states, observables, and a tension functional such that real world biodiversity trajectories can be mapped into a space where distance to collapse and distance to recovery become explicit and testable.

### 4.2 Low tension regime

In a low tension regime, states `m` in `M_reg` satisfy:

* `DeltaS_loss(m)` is small, meaning diversity is near the chosen baseline for that region and biome,
* `DeltaS_recovery(m)` is small, meaning recovery is feasible or largely complete,
* `G_driver(D_driver(m))` is moderate or low, meaning drivers are within ranges known to be compatible with sustained biodiversity.

Under these conditions:

```txt
Tension_Bio(m) is small
```

and small perturbations in drivers or state are expected to lead to bounded changes in diversity and recovery outcomes.

### 4.3 High tension regime and risk tail

In a high tension regime, at least one of the following holds:

* `DeltaS_loss(m)` is large, signalling strong biodiversity loss relative to baseline,
* `DeltaS_recovery(m)` is large, signalling that recovery is blocked or only possible under extreme interventions,
* `G_driver(D_driver(m))` is large, signalling strong and persistent drivers.

In such states:

```txt
Tension_Bio(m) is large
```

and the system resides in the risk tail of possible configurations, where:

* additional shocks or continued driver pressure can push diversity below critical levels,
* recovery paths may only exist through costly or unlikely coordinated interventions.

The canonical question can be restated at the effective layer as:

> For global and regional ecosystems, which combinations of drivers and states keep `Tension_Bio` in a manageable band where recovery is possible, and which combinations push systems into a band where tension remains high and extinction or functional collapse becomes structurally embedded?
...
These worlds are scenario classes, not claims about the actual world.

### 5.1 World T (manageable drivers, recovery possible)

In World T, typical states `m_T` in `M_reg` exhibit:

1. Bounded driver intensity

   * The components of `D_driver(m_T)` stay within ranges observed under strong conservation and climate mitigation policies.
   * Long term averages of `D_land`, `D_exploit`, and `D_pollution` remain below thresholds consistent with stable or recovering ecosystems.

2. Partial and full recovery

   * After disturbances, `DeltaS_loss(m_T)` can be reduced over time by natural and assisted recovery.
   * `DeltaS_recovery(m_T)` decreases as restoration efforts and reduced drivers take effect.

3. Tension patterns

   * `Tension_Bio(m_T)` may spike during disturbance or overshoot events but tends to return toward a moderate band as recovery proceeds.
   * For many regions, the time spent in high tension bands is limited compared to the time spent in moderate or low tension bands.

### 5.2 World F (runaway drivers, recovery blocked)

In World F, typical states `m_F` in `M_reg` exhibit:

1. Persistent high drivers

   * `D_driver(m_F)` has one or more components that remain high for long periods, for example sustained habitat conversion, intense exploitation, or severe climate anomalies.
   * These drivers are not brought back into safe operating bands.

2. Runaway biodiversity loss

   * `DeltaS_loss(m_F)` grows and remains high in many regions, indicating widespread and deep biodiversity loss.
   * Extinction debts and functional losses accumulate and are not offset by recovery.

3. Blocked recovery

   * `DeltaS_recovery(m_F)` remains large because:

     * recovery pathways are structurally blocked (for example missing keystone species or fragmented landscapes),
     * drivers remain strong enough to continuously counteract recovery efforts.

4. Tension patterns

   * `Tension_Bio(m_F)` remains in high bands for extended time intervals, indicating a structurally high risk of further loss and limited recovery.
   * Occasional local successes do not significantly reduce global or regional tension.

### 5.3 Interpretive note

World T and World F are defined only in terms of observable patterns and tension scores. They do not describe any internal mapping from raw data to TU fields. They serve as:

* conceptual extremes for driver and recovery configurations,
* benchmarks for how `Tension_Bio` should behave in cases where real or simulated ecosystems clearly resemble one world more than the other.
...
* test whether a given encoding of Q095 is coherent and useful,
* distinguish between different parameter choices for `Tension_Bio`,
* reveal encodings that fail to align with empirical biodiversity loss and recovery patterns.

These experiments do not solve Q095. They only evaluate the TU encoding at the effective layer.

### Experiment 1: Global biodiversity trajectories under mixed drivers

*Goal:*
Test whether the chosen `Tension_Bio` encoding can simultaneously reflect:
...
* For each region and time window, build an effective state `m_data` that encodes:
...
The construction is performed outside TU. Within TU we treat `m_data` as given elements of `M_reg` whenever all observables are defined.

*Protocol:*

1. Fix weights `w_loss`, `w_recovery`, `w_driver` and driver coefficients `a_land`, `a_climate`, `a_exploit`, `a_pollution`, `a_invasive` before any evaluation.

2. For each region and time window with a state `m_data` in `M_reg`:

   * compute `DeltaS_loss(m_data)`,
   * compute `DeltaS_recovery(m_data)`,
   * compute `G_driver(D_driver(m_data))`,
   * compute `Tension_Bio(m_data)`.
...
4. Compare the distributions of `Tension_Bio(m_data)` across these categories.

*Metrics:*

* For each category, compute:

  * mean and variance of `Tension_Bio`,
  * fraction of states above a chosen high tension threshold `tau_high`,
  * fraction of states below a chosen low tension threshold `tau_low`.

* Compute simple separation metrics, for example:

  * difference in mean tension between categories,
  * fraction of pairwise comparisons where states in the more degraded category have higher tension than states in the less degraded category.

*Falsification conditions:*

The encoding is considered falsified at the effective layer if either of the following occurs:

1. Weak separation

   * For any choice of `tau_low` and `tau_high` with `tau_low < tau_high` that is consistent with the overall tension scale, more than a fixed fraction `f_mis` (for example 0.4) of clearly degraded states fall below `tau_low` while more than the same fraction of near intact states fall above `tau_high`.

   This means that tension fails to distinguish degraded from intact or stable states.

2. Inverted ordering

   * In more than a fixed fraction `f_inv` of pairwise comparisons (for example 0.4) where an independent assessment labels one state as more degraded than another, `Tension_Bio` assigns a strictly lower value to the more degraded state.

   This indicates that `Tension_Bio` is systematically misaligned with external assessments.

In either case, the conclusion is:

* the specific choice of `DeltaS_loss`, `DeltaS_recovery`, weights, and driver aggregation is rejected for Q095.

This does not imply any conclusion about the real world question of biodiversity loss and recovery drivers.
...
* discrete labels for recovery status that are mapped to the continuous `R_state` variable.

This is consistent with the hybrid metadata in Block 0.

*Boundary note:*

Falsifying TU encoding != solving canonical statement. This experiment can reject specific encodings of `Tension_Bio` and its components, but it does not solve the underlying scientific problem of fully explaining biodiversity loss and recovery.
...
*Goal:*
Test whether the encoding can represent hysteresis between loss and recovery in ecosystems with known regime shifts, such as lakes shifting between clear and turbid states or coral reefs shifting between coral and algal dominance.
...
* Construct a sequence of effective states `m_1, m_2, ..., m_T` in `M_reg` that:

  * follow the ecosystem along the loss path,
  * then follow it along attempted or successful recovery paths.

Each state encodes observed or reconstructed values for `B_alpha`, `B_func`, `D_driver`, and `R_state`.

*Protocol:*

1. Fix the same encoding parameters as in Experiment 1.

2. For the sequence `m_t` along the loss path:

   * compute `Tension_Bio(m_t)` at each time step.

3. For the sequence along the recovery path:

   * compute `Tension_Bio(m_t)` again at each time step, including:

     * periods where drivers are reduced but recovery is incomplete,
     * periods after interventions that successfully restore some or all functions.

4. Plot or tabulate the tension values against a simple driver summary such as `G_driver(D_driver(m_t))` for:
...
* Presence or absence of hysteresis in the tension driver plane, for example:
  ...
  * whether degraded states show high tension at driver levels where intact states had lower tension.

* Duration and magnitude of high tension bands before and after collapse.

*Falsification conditions:*

The encoding is considered falsified for hysteresis representation if:

1. No asymmetry

   * the tension driver relationship along the loss and recovery paths is essentially identical within the resolution of the encoding, despite independent evidence of hysteresis in the ecosystem.

2. Incorrect direction

   * the encoding shows lower tension during degraded states than during intact states at the same driver levels, in contradiction with the independent ecological interpretation.

In these cases, the tension functional fails to capture the asymmetry between loss and recovery that is central to Q095.

*Semantics implementation note:*

The hybrid semantics is implemented as follows:

* continuous fields for diversity indices, driver intensities, and `Tension_Bio`,
* discrete events for regime shifts and interventions, which are mapped into changes in the continuous fields over time.

No additional semantics are introduced beyond those implied by the metadata.

*Boundary note:*

Falsifying TU encoding != solving canonical statement. Even if the encoding fails to capture hysteresis in this case, the underlying scientific question about mechanisms of ecosystem hysteresis remains open.

---

## 7. AI and WFGY engineering spec

This block describes how Q095 can be used in AI and WFGY systems at the effective layer. It does not expose any deep TU generative rules.
...
1. `signal_biodiversity_risk_tail`
   ...
   * Definition: a scalar proportional to `Tension_Bio(m)` when the model handles scenarios that include explicit biodiversity and driver information.
...
3. `signal_driver_pressure_consistency`
   ...
   * Definition: a penalty when there is a mismatch between described driver levels and claimed biodiversity states. For example, high `D_driver` with high biodiversity and low `DeltaS_loss` may incur a penalty.
...
1. `BiodiversityTensionHead`
   ...
   * Interface:
     ...
     * Outputs: a small vector containing estimated tension and its components.

2. `PolicyInterventionFilter`

   * Role: a module that scores candidate policy interventions by their expected effect on `Tension_Bio(m)` for the relevant region and time horizon.
   * Interface:
     ...
     * Outputs: estimated reductions (or increases) in tension for each intervention.
...
2. Conditions
   ...
   * Baseline condition:
     ...
     * the model responds without explicit access to `Tension_Bio` or the specialised heads.
...
3. Metrics
   ...
   * Internal consistency:
     ...
     * frequency of contradictions such as stating both that biodiversity is severely degraded and that there is no risk tail concern.
...
* TU encoded setup
  ...
  * Prompt: ask the AI to answer the same question but explicitly:
    ...
    * to treat biodiversity states as having a tension score that increases when loss is large and recovery is blocked,
  ...
  * Observation: note whether the answer:
    ...
    * uses a notion of high tension vs low tension states.
...
* What to log
  ...
  * Both prompts, both answers, and any auxiliary tension estimates, without revealing any internal generative TU rules.

---

## 8. Cross problem transfer template

This block lists reusable components produced by Q095 and their direct reuse targets.
...
1. ComponentName: `BiodiversityTensionScore`
   ...
   * Minimal interface:
     ...
     output:
       tension_value = Tension_Bio(m) >= 0
     ```

   * Preconditions:

     * the state descriptor must correspond to some `m` in `M_reg`,
     * the driver intensities and diversity indicators must be coherent and finite.
...
3. ComponentName: `AnthroDriverCouplingKernel`
   ...
   * Preconditions:
     ...
     * mapping from activity descriptors to driver intensities is calibrated for the domain.
...
1. Q080 (Limits of biosphere adaptability)
   ...
   * Why it transfers: Q080 needs a scalar or low dimensional measure of how close ecosystems are to loss thresholds and how much recovery capacity remains.
   * What changes: thresholds for acceptable tension may be stricter, and the focus shifts from local ecosystems to global aggregations of tension.
...
2. Q098 (Anthropocene system dynamics)
   ...
   * What changes: human activity vectors become a central part of the state, and the kernel may be extended to include economic and technological variables.
...
This block explains how Q095 is positioned on the TU verification ladder and what next steps are measurable and compatible with the effective layer.

### 9.1 Current levels

* E_level: E1

  * A coherent effective encoding of drivers, loss, recovery, and a risk tail tension functional has been specified.
...
* N_level: N1
  ...
  * World T and World F counterfactuals are defined qualitatively.

### 9.2 Next measurable step toward E2

To move from E1 to E2, the following steps are natural and measurable:

1. Reference implementation of observables

   * Construct a simple reference library that:

     * maps published biodiversity and driver indices into `B_alpha`, `B_func`, `D_driver`, `R_state`,
     * computes `DeltaS_loss`, `DeltaS_recovery`, and `Tension_Bio` for a set of regions and time windows.

   * Publish the resulting tension summaries for independent inspection.
...
2. Empirical validation on case studies
   ...
   * Check the falsification conditions described in Block 6 and report whether the encoding passes or fails for those datasets.

Both steps remain at the effective layer, because they operate on aggregated observables and do not reveal any TU deep generative rules.
...
* Can we define a number called `Tension_Bio` that is:
