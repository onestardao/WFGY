...
2. Evidence of new physics, non-trivial cosmic topology, large unmodeled systematics, or deeper mis-specification of the cosmological model.
...
* However, several large-scale features in the real sky appear unusual when compared to naive isotropic Gaussian expectations with simple analysis choices.
...
* Others argue that the combined pattern is unlikely under Lambda-CDM and may hint at new physics or non-trivial global structure.
...
3. It provides a testbed for Tension Universe encodings that must handle:
   ...
   * finite ensembles of mock skies vs a unique real sky,
   * rare-event statistics and look-elsewhere penalties,
   * the distinction between model mis-specification and statistical fluke.
4. It feeds downstream into other cosmological tension nodes (for example Hubble constant tension, multiverse/anthropic arguments) by constraining which combinations of model and data treatments are viable.
...
This block records the position of Q046 in the BlackHole graph, using Q001–Q125 as nodes. Each edge is labeled with a one-line reason that points to specific components or tension structures rather than vague similarity.
...
* Q050 (BH_COSMO_MULTIUNI_L3_050)
  Reason: Treats Q046’s tension functional as a selection statistic when embedding our observed sky into ensembles of universes in multiverse scenarios.
...
* Q045 (BH_COSMO_LSS_L3_045)
  Reason: Both Q045 and Q046 handle spectral_tension between high-precision stochastic-field predictions and large-scale observed patterns, but in different observable channels.
...
* Q059 (BH_CS_INFO_THERMODYN_L3_059)
  Reason: Reuses the idea of treating the CMB sky as a finite information snapshot, with Q046’s anomaly tension functional becoming a special case of information-theoretic rare fluctuation tension.

* Q123 (BH_AI_INTERP_L3_123)
  Reason: CMB anomaly detection structurally matches “rare, structured pattern in a high-dimensional field” problems in AI, so Q046’s tension tools transfer to out-of-distribution and saliency analyses.
...
* Mock-sky states, denoted `m_mock(k)`, as those representing simulated skies drawn from a specified stochastic model with a fixed parameter set and pipeline.

We do not specify how `m_real` or `m_mock(k)` are built from pixel maps or time-ordered data. We only require that for each such state the observables defined below are well defined and finite, except on explicitly declared singular sets.
...
1. Low-ℓ power spectrum observables

For each `m` in `M` and each `l` in `L_low` we define:

```txt
C_l(m)
```

as the effective estimate of the angular power at multipole `l` encoded in `m`.
...
* `f_align(m)` for quadrupole-octopole alignment;
* `f_hemi(m)` for hemispherical power asymmetry;
* `f_cold(m)` for cold spot strength and scale.

The exact contents of `F_anom(m)` and any thresholds are defined once and then held fixed. They do not depend on whether `m` is real or mock.

3. Low-ℓ spectral mismatch

We define, for each `m`:

```txt
DeltaS_spec(m) = sum over l in L_low of
                 w_l * |C_l(m) - C_l_ref| / max(C_l_ref, epsilon_C)
```
...
Both `w_l` and `epsilon_C` are chosen based on the source pack and frozen before any evaluation on real or mock skies.
...
```txt
DeltaS_feat(m) = sum over j of
                 v_j * |f_j(m) - f_j_ref| / max(sigma_j_ref, epsilon_F)
```

where:

* `f_j(m)` are components of `F_anom(m)`;
...
Again, `f_j_ref`, `sigma_j_ref`, `v_j`, and `epsilon_F` are chosen using only the baseline model and mock skies, not by tuning to the observed anomalies.
...
```txt
DeltaS_CMB(m) = w_spec * DeltaS_spec(m) + w_feat * DeltaS_feat(m)
```
...
The pair `(w_spec, w_feat)` is chosen once for Q046 and not tuned after examining the real sky.
...
```txt
T_ij(m) = S_i(m) * C_j(m) * DeltaS_CMB(m) * lambda(m) * kappa_CMB
```

where:

* `S_i(m)` represents source-like factors, such as the relative influence of primordial physics, late-time integrated effects, or systematics as encoded in the configuration.
* `C_j(m)` represents receptivity-like factors, specifying how sensitive different reasoning or decision channels are to anomaly-related mismatch.
* `DeltaS_CMB(m)` is the combined mismatch defined above.
* `lambda(m)` is a bounded scalar describing the local convergence state of the associated reasoning process (for example convergent vs divergent mode), consistent with the TU core.
...
The exact index sets of `i` and `j` are not needed at the effective layer. It is sufficient that for all relevant `i`, `j`, and `m` in the regular domain, `T_ij(m)` is well defined and finite.

### 3.4 Invariants and refinement schedule
...
* maximum multipole `L_max_low(k)`,
* number of mock skies `N_mock(k)`,
* details of the foreground mask and smoothing scale.

For each `k` we define a refinement-specific invariant:

```txt
I_CMB(m; k) = DeltaS_CMB(m; k)
```

where `DeltaS_CMB(m; k)` is computed using only observables available at the `k`th refinement level.

We commit to an **admissible encoding class** for Q046:

* The feature set `F_anom`, weight sets `{w_l}`, `{v_j}`, and the mapping from raw summaries to `C_l(m)` and `F_anom(m)` are fixed once for all `k`.
* The refinement schedule can increase resolution (for example expand `L_low`) and ensemble size, but it does not introduce new anomaly features or retune weights based on the real sky.

This admissible class ensures that rare-event significance is not artificially inflated by adding statistics until one appears extreme.
...
```txt
S_sing = { m in M :
           any C_l(m) or f_j(m) is undefined,
           or any DeltaS_spec(m) or DeltaS_feat(m) is not finite }
```

We then restrict anomaly-tension analysis to the regular domain:
...
* All definitions of `DeltaS_spec(m)`, `DeltaS_feat(m)`, `DeltaS_CMB(m)`, and `T_ij(m)` are taken to be “out of domain” on `S_sing`, not as evidence for or against any cosmological model.
...
We define the CMB anomaly tension functional:

```txt
Tension_CMB(m; k) = alpha * DeltaS_spec(m; k) + beta * DeltaS_feat(m; k)
```

where:

* `alpha > 0` and `beta > 0` are fixed constants chosen once for Q046 and held fixed across all experiments.
* `DeltaS_spec(m; k)` and `DeltaS_feat(m; k)` are the refinement-level mismatch measures defined in Block 3.

For all `m` in `M_reg` and all `k` in the refinement schedule, we have:
...
At the effective layer, a **low-tension world** for Q046 satisfies:
...
for all `k >= k_0`, given the chosen admissible encoding and baseline model.
...
A **high-tension world** for Q046 satisfies:

> For any admissible encoding in the fixed class and any reasonable Lambda-CDM baseline compatible with the wider cosmological data set, the real-sky state `m_real` eventually exhibits persistent anomaly tension beyond agreed bounds.

Formally, there exists a `delta_CMB > 0` such that for all encodings in the admissible class and for all sufficiently fine refinements `k` we have:

```txt
Tension_CMB(m_real; k) >= delta_CMB
```

and `delta_CMB` cannot be driven arbitrarily close to zero by refining `k` without leaving the admissible encoding class or contradicting independent cosmological constraints.

Q046, as an effective-layer problem, does not assert which principle is realized in our universe. It encodes the structure needed to:

* define tension in a controlled way,
* test specific encodings and baseline assumptions,
* track how evidence accumulates across refinement levels.

---

## 5. Counterfactual tension worlds

We describe two counterfactual worlds for Q046, entirely at the effective layer.
...
* World F: anomalies remain strongly atypical under any reasonable baseline and admissible encoding, suggesting new physics or severe model mis-specification.
...
1. **Low-ℓ spectra:**
   ...
   once the baseline `C_l_ref` and mask choices are fixed.
...
3. **Cross-experiment consistency:**
   ...
   When the same encoding is applied to independent experiments (for example WMAP and Planck), the corresponding states `m_real_T_WMAP` and `m_real_T_Planck` yield similar tension values, up to known noise and calibration differences.
...
4. **Global tension behavior:**
   ...
   for `k >= k_0`, with `epsilon_CMB` as in Block 4, and variance across experiments remains compatible with baseline model expectations.

### 5.2 World F (genuine anomaly physics or mis-specification, high anomaly tension)

In World F:

1. **Persistent low-ℓ discrepancies:**
   ...
   for all `k >= k_1`, with `delta_spec > 0` that cannot be removed without changing the baseline model or leaving the admissible encoding class.
...
3. **Cross-experiment persistence:**
   ...
   Independent experiments, when encoded in the same admissible class, yield tension values that cluster in a high-tension band for the real sky, while mock-sky ensembles rarely occupy this band.
...
4. **Global high tension:**
   ...
   with `delta_CMB > 0` that remains robust under refinements and analysis variations consistent with the admissible class.

### 5.3 Interpretive note

These worlds are not deep generative models. They do not specify fundamental physics or simulation pipelines. They only describe:

* how effective observables behave across refinements,
* how tension bands differ in low- vs high-tension regimes,
* how consistency across experiments is interpreted.

Q046 does not assert that our universe is in World T or World F; it sets up a framework where that question can be investigated in a controlled, falsifiable way.
...
* test the coherence of the Q046 encoding,
* distinguish between different anomaly interpretations,
* falsify specific choices of baseline model or encoding parameters.

These experiments do not, by themselves, prove or disprove Lambda-CDM. They test Q046’s effective-layer encoding and related assumptions.

### Experiment 1: Mock-sky ensemble vs real-sky tension distribution
...
* Fix a foreground mask, smoothing scale, and map-making pipeline common to real and mock skies.
* Select a refinement level `k` with specified `L_max_low(k)` and number of mock skies `N_mock(k)`.
...
3. Process the real sky through the same pipeline to obtain `m_real` and compute `Tension_CMB(m_real; k)`.
4. Construct the empirical distribution of `Tension_CMB` over the mock ensemble and locate the quantile of `Tension_CMB(m_real; k)` within this distribution.
5. Repeat for several increasing values of `k`, using the same admissible encoding and baseline model.

*Metrics:*

* The empirical cumulative distribution function of `Tension_CMB` for mock skies at each `k`.
* The quantile position `q_real(k)` of `Tension_CMB(m_real; k)` within the mock distribution.
* The stability of `q_real(k)` across refinement levels.

*Falsification conditions:*

* If, after pre-specifying an acceptable tail probability `p_star` (for example `p_star = 0.01`) and accounting for a fixed set of look-elsewhere corrections, the quantile `q_real(k)` lies in a region with `q_real(k) <= p_star` for all sufficiently large `k`, and this behavior is robust under reasonable choices of mask, then the combination of:

  * baseline model,
  * reference statistics,
  * and Q046 encoding (weights, feature set, refinement schedule)
    is flagged as high-tension and considered falsified as a coherent “low-tension world” description.
* If small, ad hoc changes in encoding parameters (for example reweighting `w_l`, `v_j`) can move `q_real(k)` from deep tail to typical region without clear physical justification, the encoding is considered unstable and rejected in its current form.

*Semantics implementation note:*

All quantities are treated as continuous-field summaries (for example averaged over pixelized skies but interpreted in a continuum limit sense), consistent with the continuous field_type in the metadata. No discrete-only or hybrid implementations are introduced in this experiment.

*Boundary note:*

Falsifying TU encoding != solving canonical statement. This experiment can reject specific combinations of baseline model and Q046 encoding, but it does not by itself prove or disprove Lambda-CDM as a fundamental theory.
...
Test whether the Q046 encoding yields consistent anomaly tension assessments across independent CMB experiments when applied with the same admissible encoding and baseline assumptions.
...
* Fix a single admissible encoding (same `L_low`, feature set, weights, and refinement schedule) compatible with all experiments.

*Protocol:*

1. Process each experiment’s data through its standard cleaning and calibration pipeline, then map to a common representation compatible with the Q046 encoding, producing states `m_real_WMAP`, `m_real_Planck`, and so on in `M_reg`.
2. For each state and each refinement level `k`, compute `Tension_CMB(m_real_exp; k)`.
...
4. Compare:
   ...
   * the relative position of these values within the corresponding mock distributions.

*Metrics:*

* Differences `|Tension_CMB(m_real_WMAP; k) - Tension_CMB(m_real_Planck; k)|` as functions of `k`.
* For each experiment, the quantile positions of real-sky tension within its own mock tension distribution.
* Coherence between experiments in terms of whether they classify the real sky as low tension, moderate tension, or high tension.

*Falsification conditions:*

* If, for a fixed baseline model and admissible encoding, one experiment consistently assigns the real sky to a low-tension band while another assigns it to a high-tension band, and this discrepancy cannot be explained by known differences in noise, resolution, or sky coverage, then the encoding is considered incoherent and rejected.
* If plausible tweaks to experiment-specific systematics can flip the tension classification without materially changing the underlying sky features, the Q046 encoding is flagged as too sensitive to instrument-level details and must be revised.
...
Falsifying TU encoding != solving canonical statement. Agreement or disagreement across experiments under this encoding tests internal consistency and model assumptions, but does not by itself settle the global CMB anomalies question.
...
2. `signal_cmb_anomaly_feature_clarity`
   ...
   * Purpose: train the model to separate different anomaly channels rather than conflating them into a single vague notion of “weird sky”.

3. `signal_cross_world_separation`

   * Definition: a signal measuring how distinctly the model maintains separate reasoning tracks when prompted under “anomalies are flukes” vs “anomalies require new physics” assumptions, as reflected in predicted `Tension_CMB`.
...
4. `signal_dataset_consistency`
   ...
   * Purpose: align the model’s explanations with the cross-experiment consistency constraints of Experiment 2.
...
1. `CMBAnomalyTensionHead`
   ...
   * Interface:
     ...
     * Inputs: condensed embeddings of the current context, plus explicit tags specifying baseline model and experiment.
     * Outputs: `(tension_scalar, tension_components_vector)`.

2. `IsotropyConsistencyFilter`

   * Role: a filter that checks whether proposed explanations maintain or break statistical isotropy in ways compatible with Q046’s admissible encoding.
   * Interface:
...
1. **Task family:**
   ...
   * Explain and assess CMB anomalies under different stated assumptions:
     ...
     * extended models (for example non-trivial topology, anisotropic inflation).

2. **Conditions:**

   * Baseline condition:

     * Model has no explicit Q046 modules or tension signals.
   * TU condition:

     * Model uses CMBAnomalyTensionHead and IsotropyConsistencyFilter to structure reasoning and generate auxiliary tension outputs.

3. **Metrics:**

   * Consistency of answers across logically related prompts (for example does the model keep its characterization of anomalies stable when assumptions are stated explicitly vs implicitly).
   ...
   * Alignment between model’s qualitative narrative and quantitative Q046 tension assessments.

### 7.4 60-second reproduction protocol

A minimal protocol for external users to experience Q046’s effect in an AI system.
...
* Record the answer from the baseline system (no explicit Q046 modules).
* Observe whether anomalies are:

  * clearly enumerated,
  * connected to statistical significance,
...
* Simple structured metadata (for example which anomalies were mentioned, how significance was described).
...
1. ComponentName: `CMB_LowL_TensionVector`
   ...
   * Type: observable bundle / functional
   * Minimal interface:

     * Inputs: low-ℓ power estimates `C_l(m)` for `l in L_low`, anomaly feature vector `F_anom(m)`, and a reference set from the source pack.
     * Output: a finite-dimensional vector of normalized mismatch scores, together with a scalar `DeltaS_CMB(m)`.
   * Preconditions:

     * Inputs must come from states in `M_reg` with clearly specified baseline model and encoding parameters.
...
2. ComponentName: `CMB_AnomalyExperiment_Template`
   ...
   * Minimal interface:
     ...
     * Inputs: baseline cosmological model specification, ensemble of mock-sky states, real-sky state, and refinement schedule.
     * Output: a standardized plan for computing mock and real tension distributions and for applying falsification conditions.
   * Preconditions:

     * All inputs must use the same admissible encoding and refinement definitions.

3. ComponentName: `CMB_AnomalyWorlds_Spec`

   * Type: field / scenario descriptor
   * Minimal interface:

     * Inputs: encoding parameters and mock ensemble statistics.
...
1. Q048 (Hubble constant tension)
   ...
   * What changes:
     ...
     * Additional observables (for example acoustic peak structure) are added to the observable bundle, but the low-ℓ anomaly part and its tension mapping remain the same.
...
2. Q050 (Multiverse and anthropic selection)
   ...
   * Why it transfers:
     ...
     * Multiverse scenarios often treat our observed sky as one realization from a huge ensemble, and the anomaly tension acts as a selection criterion or weighting factor.
   * What changes:
...
3. Q043 (Inflation origin)
   ...
   * Why it transfers:
     ...
     * Specific inflation models predict characteristic modifications to low-ℓ power and anomaly statistics, so the tension vector provides a compact way to compare model classes.
   * What changes:
...
* E_level: E1
  ...
  * The effective encoding of CMB anomalies as a spectral_tension problem is specified in terms of:
    ...
    * state space `M`,
    * observables `C_l(m)` and `F_anom(m)`,
    * mismatch measures `DeltaS_spec`, `DeltaS_feat`, and `DeltaS_CMB`,
    * a discrete refinement schedule and admissible encoding class.
...
1. **Implemented pipeline:**
   ...
   * A publicly documented code and data pipeline that:
     ...
     * computes `C_l(m)`, `F_anom(m)`, and `Tension_CMB(m; k)` for specified `k`,
     * processes both real and mock skies using the same encoding.
...
2. **Published tension summary:**
   ...
   * This summary must be sufficient for independent groups to replicate or challenge the encoding.
...
* The canonical example of how TU handles problems where:

  * there is a single observed field realization,
  * a well-defined stochastic baseline,
  * and subtle issues of rare-event statistics and model selection.
* A gateway node connecting cosmological tensions (for example Hubble constant tension, sigma_8 tension) with other spectral_tension problems in physics and information theory.
* A template for AI systems that must reason about high-precision data vs model mismatches without collapsing into overconfident claims or vague hand-waving.
...
The cosmic microwave background is the afterglow of the early universe, spread across the whole sky. It is almost perfectly uniform, but not exactly: there are tiny temperature variations that form a sort of “speckle pattern” on the sky.
...
When we actually look at the data from satellites like WMAP and Planck, we find:

* Over most angular scales, the pattern matches Lambda-CDM very well.
* On the very largest scales, however, there are some “odd” features:

  * certain large-scale waves are weaker than expected,
  * some patterns look unusually aligned,
  * one region seems unusually cold.

The CMB anomalies problem asks whether these oddities are:

* just a rare but acceptable roll of the cosmic dice, or
* a sign that the model is missing something important.

In the Tension Universe view, we do not jump straight to declaring victory or failure. Instead, we:
...
3. Combine the differences into a single “tension” score.

If the real sky’s tension score looks similar to most simulated skies, we say the anomalies are low tension under that model and encoding. If the real sky’s score sits far out in the tail, and stays there even when we refine the analysis in reasonable ways, we say the anomalies create high tension for that combination of model and assumptions.

Q046 does not decide which is the final truth. It builds a careful framework to:

* define what we mean by “anomaly” and “tension”,
* test specific choices of model and analysis,
* reuse the same ideas in other cosmology and AI problems where rare patterns might or might not signal something deep.
